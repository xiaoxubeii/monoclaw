...
export interface TeamEventMap {
  'team-changed': (team: VirtualTeam) => void;
  'team-removed': (teamId: string) => void;
  'runtime-changed': (snapshot: TeamRuntimeSnapshot) => void;
  'task-changed': (task: TeamTask) => void;
  'log': (entry: TeamAuditLogEntry) => void;
}

export interface PersistedTeamStore {
  version: 1;
  teams: VirtualTeam[];
}
