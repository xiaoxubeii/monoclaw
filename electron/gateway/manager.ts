 ...
 * Monoclaw app-attribution headers (HTTP-Referer, X-Title) for OpenRouter
...
(function () {
  ...
  if (globalThis.__monoclawFetchPatched) return;
  globalThis.__monoclawFetchPatched = true;

  globalThis.fetch = function monoclawFetch(input, init) {
    var url =
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__monoclawPatched) {
        cp.__monoclawPatched = true;
...
export class GatewayManager extends EventEmitter {
  ...
  private async initDeviceIdentity(): Promise<void> {
    ...
    try {
      const identityPath = path.join(app.getPath('userData'), 'monoclaw-device-identity.json');
  ...
  private async startProcess(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Inject fetch preload so OpenRouter requests carry Monoclaw headers.
