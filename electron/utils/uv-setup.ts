...
let disableUvMirrorForSession = false;

function isMirrorArtifactNotFoundError(error: unknown): boolean {
  const message = String(error || '');
  return message.includes('registry.npmmirror.com') && message.includes('404');
}
...
export async function setupManagedPython(): Promise<void> {
  ...
  const alreadyReady = await isPythonReady();
  if (alreadyReady) {
    logger.info('Python 3.12 is already discoverable via uv; skipping install step.');
    return;
  }

  const uvEnv = disableUvMirrorForSession ? {} : await getUvMirrorEnv();
  ...
  try {
    ...
    if (hasMirror) {
      if (isMirrorArtifactNotFoundError(firstError)) {
        disableUvMirrorForSession = true;
        logger.warn('UV mirror returned 404 for this Python artifact; disabling mirror for current app session.');
      }
