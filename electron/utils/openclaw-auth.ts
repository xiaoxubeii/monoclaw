 ...
 * Writes API keys to OpenClaw state dir agents/main/agent/auth-profiles.json
...
function getAgentRuntimeDir(agentId = 'main'): string {
  return join(getAgentsDir(), agentId, 'agent');
}
...
function getAuthProfilesPath(agentId = 'main'): string {
  return join(getAgentRuntimeDir(agentId), AUTH_PROFILE_FILENAME);
}
...
export async function removeProviderFromOpenClaw(provider: string): Promise<void> {
  ...
  for (const id of agentIds) {
    const modelsPath = join(getAgentRuntimeDir(id), 'models.json');
    try {
 ...
 * Writes to OpenClaw state config openclaw.json
 */
export async function setOpenClawDefaultModel(
  ...
  syncMainAgentModel(config, model);
...
/**
 * Keep the explicitly pinned "main" agent model aligned with the selected
 * default provider model.
 */
function syncMainAgentModel(config: Record<string, unknown>, model: string): void {
...
/**
 * Write the Monoclaw gateway token into OpenClaw state openclaw.json.
 */
...
/**
 * Ensure browser automation is enabled in OpenClaw state openclaw.json.
 */
...
export async function updateAgentModelProvider(
  ...
  for (const agentId of agentIds) {
    const modelsPath = join(getAgentRuntimeDir(agentId), 'models.json');
...
/**
 * Sanitize OpenClaw state openclaw.json before Gateway start.
 *
 ...
 * OpenClaw/Monoclaw versions or external tools.
 *
