...
| Task | Command |
|------|---------|
| Install deps + ensure uv | `pnpm run init` |
...
- **`pnpm run init`**: This runs `pnpm install` followed by `pnpm run uv:ensure`. If a bundled binary already exists (or `uv` is available in system `PATH`), it skips download. Run `pnpm run uv:download` when you need bundled binaries for packaging.
