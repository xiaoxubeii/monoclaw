...
const PENDING_FINAL_FALLBACK_MS = 15_000;
const SUPPRESSED_RUN_ID_CACHE_MAX = 128;
const _suppressedRunIdSet = new Set<string>();
const _suppressedRunIdOrder: string[] = [];

function suppressRunId(runId: string): void {
  const trimmed = runId.trim();
  if (!trimmed || _suppressedRunIdSet.has(trimmed)) return;
  _suppressedRunIdSet.add(trimmed);
  _suppressedRunIdOrder.push(trimmed);
  while (_suppressedRunIdOrder.length > SUPPRESSED_RUN_ID_CACHE_MAX) {
    const stale = _suppressedRunIdOrder.shift();
    if (stale) _suppressedRunIdSet.delete(stale);
  }
}
...
function armPendingFinalHistoryPoll(get: () => ChatState): void {
  clearHistoryPoll();
...
const VOICE_REQUEST_PATTERNS: RegExp[] = [
  /\b(tts|text[\s-]?to[\s-]?speech|voice(?:\s+reply|\s+response)?|audio(?:\s+reply|\s+response)?|read(?:\s+it|\s+this|\s+that)?\s+aloud|speak(?:\s+it|\s+this|\s+that)?|say\s+it\s+aloud)\b/i,
  /(语音|朗读|读出来|念出来|播报|配音|语音回复)/i,
];

const SESSION_STATUS_REQUEST_PATTERNS: RegExp[] = [
  /\b(session[_\s-]?status|runtime status|diagnostic(?:s)?|debug|current model|model are you using|which model|current time|current date|timezone|token usage|context usage|queue depth|session info)\b/i,
  /(当前|现在).*(模型|时间|日期|时区|会话|上下文|状态|运行时|令牌|token|用量)/i,
  /(模型|时间|日期|时区|会话|上下文|状态|运行时|令牌|token|用量).*(是什么|多少|情况)/i,
];

type GuardedToolName = 'session_status' | 'tts';
...
function hasExplicitVoiceIntent(text: string): boolean {
  return matchesAnyPattern(text, VOICE_REQUEST_PATTERNS);
}

function hasExplicitSessionStatusIntent(text: string): boolean {
  return matchesAnyPattern(text, SESSION_STATUS_REQUEST_PATTERNS);
}

function getLatestUserMessageText(messages: RawMessage[]): string {
...
function detectUnexpectedGuardedToolCall(message: unknown, latestUserText: string): GuardedToolName | null {
  const toolUses = extractToolUseUpdates(message);
  if (toolUses.length === 0) return null;

  for (const tool of toolUses) {
    const name = tool.name.trim().toLowerCase();
    if (name === 'tts' && !hasExplicitVoiceIntent(latestUserText)) return 'tts';
    if (name === 'session_status' && !hasExplicitSessionStatusIntent(latestUserText)) return 'session_status';
  }
...
function getToolResultText(message: RawMessage): string {
  const contentText = getMessageText(message.content).trim();
  if (contentText) return contentText;

  const details = message.details;
  if (details && typeof details === 'object') {
    const statusText = (details as Record<string, unknown>).statusText;
...
function buildPendingFinalFallback(
  rawMessages: RawMessage[],
  userMsTs: number,
  options?: { requireStale?: boolean },
): RawMessage | null {
  const requireStale = options?.requireStale !== false;
  const isAfterUserMsg = (msg: RawMessage): boolean => {
    if (!userMsTs || !msg.timestamp) return true;
    return toMs(msg.timestamp) >= userMsTs;
  };

  const recentToolResult = [...rawMessages].reverse().find((msg) => {
    if (!isToolResultRole(msg.role)) return false;
    if (!isAfterUserMsg(msg)) return false;
    return getToolResultText(msg).length > 0;
  });

  if (!recentToolResult) return null;

  const freshUntil = recentToolResult.timestamp ? toMs(recentToolResult.timestamp) : _lastChatEventAt;
  if (requireStale && (Date.now() - Math.max(_lastChatEventAt, freshUntil)) < PENDING_FINAL_FALLBACK_MS) {
    return null;
  }

  const content = getToolResultText(recentToolResult);
  if (!content) return null;

  return {
    role: 'assistant',
    id: `fallback-toolresult-${recentToolResult.id || recentToolResult.toolCallId || 'latest'}`,
    timestamp: recentToolResult.timestamp,
    content,
  };
}
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadHistory: async (quiet = false) => {
    ...
    try {
      ...
      if (result.success && result.result) {
        ...
        let visibleMessages = finalMessages;
        ...
        if (pendingFinal || get().pendingFinal) {
          if (recentAssistant) {
            ...
            if (fallbackMessage && !visibleMessages.some((msg) => msg.id === fallbackMessage.id)) {
              visibleMessages = [...visibleMessages, fallbackMessage];
            }
            if (fallbackMessage) {
              clearHistoryPoll();
              set({
        ...
        if (!recentAssistant) {
          ...
          if (lastRawMessage && isToolResultRole(lastRawMessage.role)) {
            const orphanFallback = buildPendingFinalFallback(rawMessages, userMsTs, { requireStale: false });
            if (orphanFallback && !visibleMessages.some((msg) => msg.id === orphanFallback.id)) {
              visibleMessages = [...visibleMessages, orphanFallback];
            }
          }
        }

        set({ messages: visibleMessages, thinkingLevel, loading: false });
      } else {
  ...
  sendMessage: async (text: string, attachments?: Array<{ fileName: string; mimeType: string; fileSize: number; stagedPath: string; preview: string | null }>) => {
    ...
    const checkStuck = () => {
      ...
      if (state.pendingFinal) {
        if (Date.now() - _lastChatEventAt >= PENDING_FINAL_FALLBACK_MS) {
          void state.loadHistory(true);
        }
  ...
  handleChatEvent: (event: Record<string, unknown>) => {
    ...
    if (runId && _suppressedRunIdSet.has(runId)) return;
    ...
    // Guardrail: block known noisy tool calls unless the user explicitly asked.
    // This prevents local small models from getting stuck in tool-only loops.
    if (event.message && typeof event.message === 'object') {
      const latestUserText = getLatestUserMessageText(get().messages);
      const blockedTool = detectUnexpectedGuardedToolCall(event.message, latestUserText);
      if (blockedTool) {
        if (runId) suppressRunId(runId);
        clearHistoryPoll();
        clearErrorRecoveryTimer();
        const nowSec = Date.now() / 1000;
        const guardMessageId = `guard-${blockedTool}-${runId || Date.now()}`;
        const guardText = blockedTool === 'session_status'
          ? 'Blocked unexpected `session_status` tool call. This tool is reserved for explicit runtime/model/time diagnostics.'
          : 'Blocked unexpected `tts` tool call. Ask explicitly for voice/audio output if you want TTS.';

        set((s) => ({
          messages: s.messages.some((m) => m.id === guardMessageId)
            ? s.messages
            : [...s.messages, { role: 'assistant', content: guardText, timestamp: nowSec, id: guardMessageId }],
          sending: false,
          activeRunId: null,
          streamingText: '',
          streamingMessage: null,
